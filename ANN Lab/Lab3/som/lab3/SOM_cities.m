    ...
    for j=1:types
        % For each cities: pick out the corresponding row from the props matrix.
         ...
         if lower<1
             ...
             weight(1:upper,:) = weight(1:upper,:)+step_size*bsxfun(@minus,p,weight(1:upper,:));
         elseif upper>types
             weight(1:(upper-types),:) = weight(1:(upper-types),:)+step_size*bsxfun(@minus,p,weight(1:(upper-types),:));
             weight(lower:types,:) = weight(lower:types,:)+step_size*bsxfun(@minus,p,weight(lower:types,:));
         else
